...
{
    QString ret = QString("Nugget:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non NGT unit input in lnReqAmount, we generate the URI with NGT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(NuggetUnits::format(NuggetUnits::NGT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
